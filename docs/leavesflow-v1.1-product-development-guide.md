# LeavesFlow V1.2 产品开发与维护文档

版本日期：2026-05-17
适用分支：`v1`
适用范围：LeavesFlow V1.2 Web + API 当前实现
文档状态：V1.2 当前产品与代码基线说明。文件名保留 `v1.1` 是为了兼容已有引用，正文以 V1.2 为准。
...
V1.2 仍是单文件主应用结构。后续如果继续扩展，建议拆分组件，但拆分时要保持数据流和命名一致。
...
LeavesFlow V1.2 已经从演示型 MVP 推进到真实用户闭环：用户注册后建立长期画像，围绕目标选择任务偏好，系统通过真实 AI 中转生成可执行任务路径，路径和打卡状态持久化保存，完成经验继续沉淀为可复用的能力 Prompt 资产。
